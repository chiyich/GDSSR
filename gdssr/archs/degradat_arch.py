...
class DEResNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_degradation):
            ...
            if pooling[i] =="max":
                self.pooling.append(nn.AdaptiveMaxPool2d(1))
            else:
                self.pooling.append(nn.AdaptiveAvgPool2d(1))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_degradation):
            ...
            feat = self.pooling[i](feat)
            feat = feat.squeeze(-1).squeeze(-1)
            # for i in range(self.num_degradation):
            degrees_feat.append(feat) #B,nf
            degrees.append(self.fc_degree[i](feat).squeeze(-1))
        if self.return_feat:
            return degrees, torch.stack(degrees_feat, dim=1) #[B,2,feat]
        else:
            return degrees
...
from gdssr.data.degradations import custom_random_add_gaussian_noise_pt, custom_random_add_poisson_noise_pt, \
    custom_random_add_gaussian_noise_pt_anchor, custom_random_add_poisson_noise_pt_anchor
from gdssr.data.transforms import paired_random_crop
from basicsr.data.degradations import random_add_gaussian_noise_pt, random_add_poisson_noise_pt


@MODEL_REGISTRY.register()
class GDSSR_NetModel(SRModel):
    """RealESRNet Model"""

    def __init__(self, opt):
        super(GDSSR_NetModel, self).__init__(opt)
    ...
    def _dequeue_and_enqueue(self):
        ...
        if not hasattr(self, 'queue_lr'):
            ...
            self.queue_lrx = torch.zeros(self.queue_size, 3, c, h2, w2).cuda()
            self.queue_anchorx = torch.zeros(self.queue_size, 2, c, h2, w2).cuda() 
        ...
        if self.queue_ptr == self.queue_size:  # full
            ...
            self.queue_lrx = self.queue_lrx[idx]
            self.queue_anchor = self.queue_anchor[idx]
            self.queue_anchorx = self.queue_anchorx[idx]
            self.queue_gt = self.queue_gt[idx]
            # get
            lq_dequeue = self.queue_lr[0:b, :, :, :, :].clone()
            anchor_dequeue = self.queue_anchor[0:b, :, :, :, :].clone()
            lqx_dequeue = self.queue_lrx[0:b, :, :, :, :].clone()
            anchorx_dequeue = self.queue_anchorx[0:b, :, :, :, :].clone()
            gt_dequeue = self.queue_gt[0:b, :, :, :, :].clone()
            # update
            self.queue_lr[0:b, :, :, :, :] = self.lq.clone()
            self.queue_anchor[0:b, :, :, :, :] = self.anchor.clone()
            self.queue_lrx[0:b, :, :, :, :] = self.lqx.clone()
            self.queue_anchorx[0:b, :, :, :, :] = self.anchorx.clone()
            self.queue_gt[0:b, :, :, :, :] = self.gt.clone()

            self.lq = lq_dequeue
            self.anchor = anchor_dequeue
            self.lqx = lqx_dequeue
            self.anchorx = anchorx_dequeue
            self.gt = gt_dequeue
        else:
            # only do enqueue
            self.queue_lr[self.queue_ptr:self.queue_ptr + b, :, :, :, :] = self.lq.clone()
            self.queue_anchor[self.queue_ptr:self.queue_ptr + b, :, :, :, :] = self.anchor.clone()
            self.queue_lrx[self.queue_ptr:self.queue_ptr + b, :, :, :, :] = self.lqx.clone()
            self.queue_anchorx[self.queue_ptr:self.queue_ptr + b, :, :, :, :] = self.anchorx.clone()
    ...
    def feed_data(self, data):
        if self.is_train and self.opt.get('high_order_degradation', True):
            ...
            outs[2] = F.interpolate(outs[2], scale_factor=scale_max, mode=mode) #scale\Blur\sinc
            outs.append(outs[0].clone().detach())
            outs.append(self.gt.clone()) #4 add GN PN JPEG
            # noise
            ...
            outs[4] = self.jpeger(outs[4], quality=jpeg_p_max.clone())
            ...
            if np.random.uniform() < 0.5:
                ...
                outs[:-1] = [filter2D(out, self.sinc_kernel) for out in outs[:-1]]
                # JPEG compression
                sample_diff_abs = torch.rand(
                    outs[0].size(0), dtype=outs[0].dtype, device=outs[0].device) * (
                                          self.opt['jpeg_diff_range'][1] - self.opt['jpeg_diff_range'][0]) \
            ...
            # clamp and round
            if self.opt['gt_usm'] is True:
                anchor_noise = self.usm_sharpener(outs[4])   # generate the sharp anchor
            else:
                anchor_noise = outs[4]
            anchor_noise = F.interpolate(anchor_noise, size=(ori_h // self.opt['scale'], ori_w // self.opt['scale']), mode='bilinear')
            anchor_blur = outs[2]
            anchor_min = self.gt
            anchor_min = F.interpolate(anchor_min, size=(ori_h // self.opt['scale'], ori_w // self.opt['scale']),
                                    mode='bilinear')
            self.anchor = torch.stack([anchor_noise,anchor_blur],dim=1)
            self.anchorx = torch.clamp((self.anchor * 255.0).round(), 0, 255) / 255.

            outs = [outs[0], outs[1], outs[3]]
            out = torch.stack(outs, dim=1)
            self.lqx = torch.clamp((out * 255.0).round(), 0, 255) / 255.

            # random crop
            gt_size = self.opt['gt_size']
            lq_size = gt_size // self.opt['scale']
            b_gt, c_gt, h_gt, w_gt = self.gt.size()
            b_lq, _, c_lq, h_lq, w_lq = self.lqx.size()
            self.gt = self.gt.unsqueeze(1).repeat(1, 5, 1, 1, 1)
            # print(self.anchor.shape)
            self.gt, (self.lq, self.anchor) = paired_random_crop(self.gt, [self.lqx, self.anchorx], gt_size, self.opt['scale'])
            # print(self.anchor.shape)
            # exit(0)
    ...
    def init_training_settings(self):
        ...
        super(GDSSR_NetModel, self).init_training_settings()

    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        # the size of self.pred_blur and self.pred_noise is torch.Size([b*3])
        self.output, (self.pred_blur, self.pred_noise) = self.net_g(self.lq, lqx=self.lqx, anchor=self.anchor, anchorx=self.anchorx)
        ...
        if self.cri_pix:
            ...
            if current_iter > self.opt['train']['pix_start']:
                l_total += l_pix
        ...
        # ranking loss
        if self.cri_ranking:
            l_anchor_noise = (torch.mean((self.pred_anchor_blur[:, 0] - target_min)**2) + torch.mean((self.pred_anchor_noise[:, 0] - target_max)**2))*self.opt['train']['rank_opt_anchor']['loss_weight']
            l_anchor_blur = (torch.mean((self.pred_anchor_blur[:, 1] - target_max)**2) + torch.mean((self.pred_anchor_noise[:, 1] - target_min)**2))*self.opt['train']['rank_opt_anchor']['loss_weight']
            #l_anchor_min = (torch.mean((self.pred_anchor_blur[:, 2] - target_min)**2) + torch.mean((self.pred_anchor_noise[:, 2] - target_min)**2))*self.opt['train']['rank_opt_anchor']['loss_weight']
            ...
            l_total += l_anchor_noise
            l_total += l_anchor_blur
            #l_total += l_anchor_min
            loss_dict['l_ranking_blur'] = l_ranking_blur
            loss_dict['l_ranking_noise'] = l_ranking_noise
            loss_dict['l_anchor_noise'] = l_anchor_noise
            loss_dict['l_anchor_blur'] = l_anchor_blur
            #loss_dict['l_anchor_min'] = l_anchor_min
        # (ranking) constraint loss
        if self.cri_constraint:
            l_constraint_blur = self.cri_constraint(self.pred_blur[:, 0], self.pred_blur[:, 2])
